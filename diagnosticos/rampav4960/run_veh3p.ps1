...
$result = Join-Path $Lab "veh3p.txt"
if (Test-Path $result) { Remove-Item $result }
$dwg = Join-Path $Lab 'e2eveh3p.dwg'
...
Start-Process -FilePath "powershell" -WindowStyle Hidden `
  -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',
    "C:\Users\juanbusper\Documents\URBANISMO\work\audit_mt\domar_dialogos.ps1",'-DuracionSeg','520')

$t0 = Get-Date
while (((Get-Date) - $t0).TotalSeconds -lt 520) {
