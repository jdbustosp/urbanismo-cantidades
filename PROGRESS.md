...
### Auditoría de redes húmedas y andenes — 2026-09-13 (Codex / BOG085CD119BDQN)
Con la copia de solo lectura `Documents\URBANISMO\work\mt_pozos505\MASTER_test.dwg` se confirmó que `MP_PUNTO_POZO_SAN` handle `4B951` (DOM41) tiene `PROFUNDIDAD=2559.65` y `COTA_CLAVE_INI=2.00`. El tramo DOM41–55, handle `9761C`, queda con 9,317.00 m³ de excavación, 9,316.16 m³ de lleno y 15,528.33 m² de entibado; es la causa dominante de la inflación de 4E. Se identificaron además 4 profundidades negativas, 4 vacías y 15 grupos de tramos sanitarios duplicados (15 filas extra). No se modificaron cotas ni entidades. Informe completo: `MT_RED_HUMEDA_Y_ANDENES_AUDITORIA_2026-09-13.md`. La fuente y el bundle instalado 5.0.6 coinciden por SHA-256. Los fixtures de andén validan ausencia de círculos, continuidad guía/toperol y empaque en bloque; falta repetir el ensayo final de tiempo/visual dentro de Civil 3D abierto porque CoreConsole no expone VLA/ActiveDocument para esta prueba.

### v5.7.10 — error ACIS 18003 al editar andén y control de excavación configurable (2026-09-21)

El registro real de las 20:52 confirmó que la edición mostrada por el usuario sí terminó: guía y toperol `build=T`, `-BLOCK OK`, bloque insertado y bloque anterior reemplazado. El `Error Code Number is 18003` venía de booleanos auxiliares de medición ejecutados después de crear el acabado. `urb:anden-measured-finish` ya no vuelve a unir/restar regiones ACIS que solo se tocan; suma las caras generadas y, si no forman una partición exacta (por superposición táctil), usa el cálculo contractual geométrico ya validado. Civil 3D 2023 real: **68 PASS, 0 FAIL**, bloque único, cero sueltas y log sin 18003. También se validó que el DCL nuevo carga y que el límite queda persistido por dibujo.
